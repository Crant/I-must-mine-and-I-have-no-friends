...
unsigned int Maths::RandNumber(int pos, unsigned int seed) //Grundfunktionen för randomisering
{
...
float Maths::SmoothStep(float t) //Default 3 gånger gångrad
{
	float inputSquared = (t * t);
	return (3.0f * inputSquared) - (2.0f * inputSquared * inputSquared);
}
float Maths::SmoothStart(float t) //Default 3 gånger gångrad
{
	return t * t * t;
}
float Maths::SmoothStop(float t) //Default 3 gånger gångrad
{
