...
bool Player::UpdateMenu(std::shared_ptr<DynamicMenu> menu)
{
	if (menu != nullptr && menu->bMenuActive)
	{
		...
		if (bHasReleasedItem)
		{
			Inventory::AddItemFromInventoryToOther(cFirstInvCheck, cSecondInvCheck, nItemPressedIndex, nLastIndex); //ÄNDRA DETTA TILL ETT EVENT SOM SEDAN OMVANDLAS TILL MSG FÖR SERVER
			bHasReleasedItem = false;
		}
